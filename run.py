...
class DetThread(QThread):
    ...
    def __init__(self):
        ...
        self.weights = './weights/test25.pt'
        self.source = '0'
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        self.model = './weights/test25.pt'
        self.det_thread = DetThread()
        self.det_thread.source = './data/images/10.jpg'
    ...
    def open_file(self):
        try:
            ...
            if not os.path.exists(open_fold):
                open_fold = os.getcwd()
            source, _ = QFileDialog.getOpenFileName(self, '选取视频或图片', open_fold, "Pic File(*.mp4 *.mkv *.avi *.flv "
                                                                          "*.jpg *.png)")
            if source:
                self.det_thread.source = source
                config['open_fold'] = os.path.dirname(source)
                config_json = json.dumps(config, ensure_ascii=False, indent=2)
                with open(config_file, 'w', encoding='utf-8') as f:
                    f.write(config_json)
            self.statusbar.showMessage('加载视频或图片：{}'.format(os.path.basename(self.det_thread.source)))
        except Exception as e:
            print(repr(e))
